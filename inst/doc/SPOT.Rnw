...
can be seen in other SPOT vignette, \href{SPOTmanual.pdf}{SPOTmanual}.
...
search space. If you want to see those, you can look them up in the .des File that the "init" step created. It is
...
spot(testFile,"run")
@
The results of that algorithm run will be written into a .res file in the project directory: \\[12pt]
\parbox{\textwidth}{\fbox{\parbox{\textwidth}{
...
What we see here, is a tree for our SPOT run. This is created by the default
...
step uses the availabe data to create a new tree for data created by any kind of 
prediction model. 
