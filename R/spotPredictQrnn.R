...
#' @export
###################################################################################
spotPredictQrnn<- function(rawB,mergedB,largeDesign,spotConfig,externalFit=NULL){	
	spotWriteLines(spotConfig$io.verbosity,1,"spotPredictQrnn started");
	spotInstAndLoadPackages("qrnn")	
	...
	if(is.null(externalFit)){
		...
		res <- qrnn.predict(as.matrix(largeDesign),fit)
	}else{
		fit<-externalFit
	...
	spotWriteLines(spotConfig$io.verbosity,1,"spotPredictQrnn finished");
