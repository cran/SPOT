	...
	###################################################################################
	spotPredictOptMultiNew <- function(startPoint,spotConfig){	
		...
		if ( class(spotConfig$seq.fitList[[1]])[1]=="rsm"){#TODO: This might not work, due to missing normalization? See spotPredictLm
			spotGetFitness <- function(x){				
				names(x)<- as.character(makeNNames(nParam));
				val<-NULL;				
				for(i in 1:length(spotConfig$seq.fitList)){
					val<-rbind(val,predict(spotConfig$seq.fitList[[i]],as.data.frame(rbind(x))));
				}			
				return(mean(val)-sd(val))
			}	
		}
		else if( class(spotConfig$seq.fitList[[1]])[1]=="gp"){#TODO: This might not work, due to missing normalization? See spotPredictLm
			spotGetFitness <- function(x){				
				names(x) <- row.names(spotConfig$alg.aroi);
				val<-NULL;				
				for(i in 1:length(spotConfig$seq.fitList)){
					val<-rbind(val,predict(spotConfig$seq.fitList[[i]],as.matrix(rbind(x))));
				}			
				return(mean(val)-sd(val))
			}	
		}
		else if( class(spotConfig$seq.fitList[[1]])[1]=="tgp"){#TODO: This might not work, due to missing normalization? See spotPredictLm
			spotGetFitness <- function(x){				
				names(x) <- row.names(spotConfig$alg.aroi);
				val<-NULL;		
				for(i in 1:length(spotConfig$seq.fitList)){
					val<-rbind(val,predict(spotConfig$seq.fitList[[i]],as.matrix(rbind(x)))$ZZ.mean);
				}			
				return(mean(val)-sd(val))
			}	
		}
