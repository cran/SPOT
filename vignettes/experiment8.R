...
progSpotHyb <- matrix(NA, nrow = reps, ncol = 2*funEvals)
for(r in 1:reps){
  set.seed(r)
  print(r)
  solHyb <- spot(x = x0,
  ...
  progSpotHyb[r, ] <- bov(solHyb$y, 2*funEvals)
}

matplot(t(progSpotHyb), type="l", col="red", lty=1, xlab="n: blackbox evaluations", ylab="best objective value", log="y")
abline(v=size, lty=2)
matlines(t(progSpot), type="l", col="gray", lty=1)
legend("topright", c("Spot", "SpotHyb"), col=c("gray", "red"), lty=1, bty="n")


experiment8 <- list(progSpot= progSpot, progSpotHyb = progSpotHyb)
save(experiment8, file="experiment8.RData")

load("/media/data/workspace/spot/vignettes/experiment8.RData")
progSpot <- experiment8$progSpot
progSpotHyb <- experiment8$progSpotHyb
ySpot <- progSpot[,ncol(progSpot)]
ySpotHyb <- progSpotHyb[,ncol(progSpotHyb)]
boxplot(ySpot, ySpotHyb)

yMeanSpot<-apply(progSpot, 2,mean)
yMeanSpotHyb<-apply(progSpotHyb, 2,mean)

x<-1:length(yMeanSpot)

plot(x, yMeanSpot, type ="l")
lines(x, yMeanSpotHyb, type ="l", col = "red")
